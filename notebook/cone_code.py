...
class ConeFinder:
    ...
    def _convexHullIsPointingUp(self, hull):
        ...
        if aspectRatio > 0.9:
            #print('Aspect ratio (%f) too low' % aspectRatio)
            return False
    ...
    def find_cones(self, img, depthImg=None):
        ...
        # Gaussian blur
        #imgThreshSmoothed = cv2.GaussianBlur(imgThreshSmoothed, (3, 3), 0)
        ...
        for (hull, area, (dMin, dMax)) in listOfHullsAndArea:
            ...
            if (len(hull) >= 3 and self._convexHullIsPointingUp(hull)):
                ...
                #print('x=%d, y=%d, w=%d, h=%d, cx=%d, cy=%d' % (x, y, w, h, image_centerX, image_centerY))
                pose = pose_data()
                ...
                pose.d = dMax - dMin
                pose.area = area
                #print('x=%d, y=%d, z=%d, ar=%f, w=%d, h=%d, d=%d' % (pose.x, pose.y, pose.z, pose.area, pose.w, pose.h, pose.d))
                poses.append(pose)
            #else:
            #    print('Skipped shape with %d points' % len(hull))

        return (img, poses, listOfCones)
...
class ConeSeeker:
    ...
    def _update_prev_poses(self):
        ...
        #print(' '.join(str(p) for p in new_pos_confs))
        # Keep only first 16 items
        self.prev_pos_confs = new_pos_confs[:16]
    ...
    def seek_cone(self, poses, imghull):
        ...
        for pose in poses:
          ...
          #frame should be 0
          pt1 = (int(iw/2 + pose.x - pose.w/2), ih - pose.y)
          pt2 = (int(iw/2 + pose.x + pose.w/2), ih - pose.y - pose.h)
          cv2.rectangle(imghull, pt1, pt2, (255,0,0), 3)
          msg_str = '%.3f' % conf
          pt1 = (int(iw/2 + pose.x - pose.w/2), ih - pose.y + 20)
